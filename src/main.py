...
def run_pipeline(img_path, target_x, target_y):
    ...
    if not (0 <= target_x <= RAW_IMG_SIZE[1] and 0 <= target_y <= RAW_IMG_SIZE[0]):
        raise ValueError("Target coordinates must lie within 800x600 image bounds.")
    ...
    ann_model.load_state_dict(torch.load(BEST_MODEL_PATH, map_location=DEVICE))
    ann_model.eval()
